...
%RUN_CORE_FIX_V01_DETAILED_AUDIT Time-series audit for v02 membrane values and links.
...
variableFile = fullfile(resultDir, 'core_fix_v02_detailed_variable_audit.csv');
interfaceFile = fullfile(resultDir, 'core_fix_v02_interface_consistency.csv');
mMemFile = fullfile(resultDir, 'core_fix_v02_mmem_timeseries.csv');
summaryFile = fullfile(resultDir, 'core_fix_v02_detailed_summary.md');
...
stateVars = ["mO2_state_kg", "mN2_state_kg", "mH2Ov_ca_state_kg", ...
    "mH2_state_kg", "mH2Ov_an_state_kg", "T_state_C", "a_memb_state"];
stateUnits = ["kg", "kg", "kg", "kg", "kg", "degC", "1"];
...
trace = table(repmat(string(P.case_id), n, 1), repmat(string(P.source_dataset), n, 1), ...
    ...
    s(:, 64), s(:, 65), s(:, 49), s(:, 50), s(:, 8), s(:, 66), s(:, 67), ...
    s(:, 68), s(:, 69), s(:, 70), s(:, 71), s(:, 72), s(:, 73), s(:, 74), ...
    s(:, 75), s(:, 76), s(:, 77), s(:, 78), s(:, 11), s(:, 9), s(:, 2), ...
    ...
    'lambda_ca', 'lambda_an', 'lambda_m', 'aCa_raw', 'aAn_raw', ...
    'aCa_limited', 'aAn_limited', 'a_memb', 'a_memb_target_raw', ...
    'a_memb_target', 'J_diff_ca_to_mem_mol_m2_s', 'J_diff_mem_to_an_mol_m2_s', ...
...
names = ["I_A", "V_cell", "Pstack_kPa_abs", "pO2_stack_kPa", ...
    ...
    "mDrag_kg_s", "mDiff_kg_s", "mMem_raw_kg_s", "mMem_limit_delta_kg_s", ...
    "aCa_raw", "aAn_raw", "aCa_limited", "aAn_limited", "a_memb", ...
    "a_memb_target_raw", "a_memb_target", "J_diff_ca_to_mem_mol_m2_s", ...
    "J_diff_mem_to_an_mol_m2_s", "dm_lambda_equiv_kg", "k_mem_eff", ...
    "tau_memb_s", "membrane_dynamic_mode"];
end
...
units = ["A", "V", "kPa", "kPa", "kPa", "kPa", "kPa", ...
    ...
    "kg/s", "kg/s", "kg/s", "kg/s", "1", "1", "1", "1", "1", ...
    "1", "1", "mol/(m2*s)", "mol/(m2*s)", "kg", "1", "s", "1"];
end
...
varTypes = [{'string', 'string'}, repmat({'double'}, 1, 30)];
t = table('Size', [0, 32], ...
    'VariableTypes', varTypes, ...
    'VariableNames', {'case_id','source_dataset','current_A','egr_fraction','time_s', ...
    'mMem_kg_s','J_drag_mol_m2_s','J_diff_mol_m2_s','J_net_mol_m2_s', ...
    'mDrag_kg_s','mDiff_kg_s','mMem_raw_kg_s','mMem_limit_delta_kg_s', ...
    'lambda_ca','lambda_an','lambda_m','aCa_raw','aAn_raw','aCa_limited', ...
    'aAn_limited','a_memb','a_memb_target_raw','a_memb_target', ...
...
fprintf(fid, '# Core Fix v02 Detailed Audit Summary\n\n');
...
lambdaRows = variableAudit(variableAudit.module == "stack_summary" & variableAudit.variable == "lambda_m", :);
aRows = variableAudit(variableAudit.module == "stack_summary" & variableAudit.variable == "a_memb", :);
rawCaRows = variableAudit(variableAudit.module == "stack_summary" & variableAudit.variable == "aCa_raw", :);
rawAnRows = variableAudit(variableAudit.module == "stack_summary" & variableAudit.variable == "aAn_raw", :);
if ~isempty(lambdaRows)
    fprintf(fid, '- Largest lambda_m last-window range: %.6g\n', max(lambdaRows.last_window_range));
end
if ~isempty(aRows)
    fprintf(fid, '- Largest a_memb last-window range: %.6g\n', max(aRows.last_window_range));
end
if ~isempty(rawCaRows)
    fprintf(fid, '- Max raw cathode water activity final value: %.6g\n', max(rawCaRows.final_value));
end
if ~isempty(rawAnRows)
    fprintf(fid, '- Max raw anode water activity final value: %.6g\n', max(rawAnRows.final_value));
end
fprintf(fid, '\n');
